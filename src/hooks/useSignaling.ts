...
import { fetchTicket, clearTicketCache } from '../lib/ticket';
        ...
        ws.onmessage = (ev) => {
          try {
            ...
            if (parsed.t === 'ready') {
              ...
              setSocketId(String(parsed.d.socketId));
              setOnline(Number(parsed.d.online));
              setStatus('ready');
              setError(null);
              attemptRef.current = 0;
              finish();
            }
            if (parsed.t === 'online') setOnline(Number(parsed.d.count));
            if (parsed.t === 'error') setError(String(parsed.d.message));
