...
export const VideoChatScreen = forwardRef<
  VideoChatScreenRef,
...
>(
  (
    ...
    const localVideoPosition = useRef(
      new Animated.ValueXY({x: width - 130, y: 20}),
    ).current;
  ...
  if (isInCall) {
    ...
    useEffect(() => {
      ...
      if (shouldDisconnectOnTabSwitch && connectionStatus !== 'disconnected') {
        console.log(
          'Auto-disconnecting from video server due to tab switch...',
        );
        disconnect();
        return;
      }
    ...
    const connectToServer = async () => {
      try {
        ...
        ws.onerror = error => {
          ...
          setError(
            'Connection lost. Please check your internet and try reconnecting.',
          );
          setConnectionStatus('disconnected');
          setIsConnected(false);
        };
        ...
        if (errorMessage.includes('Rate limit exceeded')) {
          ...
          setError(
            `Too many connection attempts. Please wait ${retryTime} seconds and try again.`,
          );
        } else if (
          errorMessage.includes('Network error') ||
          errorMessage.includes('Cannot reach annochat.social')
        ) {
          setError(
            "Can't connect to our servers. Please check your internet connection and try again.",
          );
          ...
          errorMessage.includes('Video token API endpoint not found')
        ) {
          setError(
            'Service temporarily unavailable. Our team is working on it. Please try again later.',
          );
      ...
      try {
        ...
        if (isInitiator) {
          ...
          (dataChannel as any).onopen = () => {
            console.log(
              'DataChannel open - sending initial camera state and requesting remote state',
            );
          ...
          (dataChannel as any).onmessage = (event: MessageEvent) => {
            try {
              ...
              if (msg.type === 'request-state') {
                console.log(
                  'Remote peer requested current state - sending our camera/mic status',
                );
          ...
          (peerConnection as any).ondatachannel = (event: any) => {
            ...
            (dataChannel as any).onopen = () => {
              ...
              if (dataChannel && dataChannel.readyState === 'open') {
                ...
                dataChannel.send(
                  JSON.stringify({type: 'camera', enabled: isCameraOn}),
                );
                dataChannel.send(
                  JSON.stringify({type: 'mic', enabled: isMicOn}),
                );
              }
            };
            (dataChannel as any).onmessage = (event: MessageEvent) => {
        ...
        (peerConnection as any).ontrack = (event: any) => {
          ...
          if (event.streams && event.streams[0]) {
            ...
            setIsRemoteCameraOn(hasActiveVideo);
            console.log(
              'Remote camera status on track received:',
              hasActiveVideo,
              'tracks:',
              videoTracks.length,
            );
        ...
        (peerConnection as any).onaddstream = (event: any) => {
          ...
          if (event.stream) {
            ...
            setIsRemoteCameraOn(hasActiveVideo);
            console.log(
              'Remote camera status on stream received:',
              hasActiveVideo,
              'tracks:',
              videoTracks.length,
            );
    ...
    const findMatch = async () => {
      ...
      try {
        ...
        if (hasExistingVideoStream) {
          console.log(
            'Reusing existing background camera stream, just adding audio...',
          );
          ...
          try {
            ...
            console.warn(
              'Failed to add audio track, continuing with video only:',
              audioError,
            );
          }
        } else {
          ...
          console.log(
            'Creating media stream with camera facing:',
            cameraFacing,
          );
          const stream = await mediaDevices.getUserMedia({
            video: {
        ...
        setError(
          'Unable to access camera/microphone. Please grant permissions and try again.',
        );
        setIsProcessing(false);
    ...
    const toggleCamera = async () => {
      ...
      if (localStreamRef.current && peerConnectionRef.current) {
        if (isCameraOn) {
          ...
          if (
            ...
            dataChannelRef.current.send(
              JSON.stringify({type: 'camera', enabled: false}),
            );
          }
        } else {
          ...
          try {
            ...
            if (newVideoTrack) {
              ...
              if (
                ...
                dataChannelRef.current.send(
                  JSON.stringify({type: 'camera', enabled: true}),
                );
              }
            }
          } catch (error) {
        ...
        console.log(
          'Toggling camera during search/preparation or background preview',
        );
        const videoTrack = localStreamRef.current.getVideoTracks()[0];
        ...
        if (newCameraState && !isInCall && !isSearching) {
          ...
          try {
            ...
            console.error(
              'Error creating background camera stream manually:',
              error,
            );
      ...
      if (localStreamRef.current) {
        ...
        if (audioTrack) {
          ...
          if (
            ...
            dataChannelRef.current.send(
              JSON.stringify({type: 'mic', enabled: audioTrack.enabled}),
            );
          }
        } else {
          ...
          setIsMicOn(!isMicOn);
          console.log(
            'No audio track available, just updating state:',
            !isMicOn,
          );
          // Send mic state to remote peer
          if (
    ...
    const flipCamera = async () => {
      ...
      try {
        ...
        if (newVideoTrack) {
          ...
          console.log(
            'Added new video track with facing:',
            newFacing,
            'enabled:',
            isCameraOn,
          );
          ...
          console.log(
            'Camera flip completed successfully - preview should update',
          );
        } else {
        ...
        <Animated.View style={[styles.mainContent, {opacity: fadeAnim}]}>
          ...
          <View style={styles.videoInterface}>
            <Animated.View
              ...
              <Animated.View
                style={[styles.orbitalRing1, {opacity: glowOpacity}]}
              />
            ...
            <Text style={styles.mainTitle}>
              {isSearching ? 'Scanning Universe...' : 'Video Discovery'}
            </Text>
          ...
          <View style={styles.centerActionContainer}>
            <TouchableOpacity
              style={[
                ...
                (isButtonDisabled || (status !== 'Ready' && !isSearching)) &&
                  styles.quickMatchButtonDisabled,
              ]}
              onPress={
                connectionStatus === 'disconnected'
              ...
              <Text
                style={[
                  ...
                  (isButtonDisabled || isProcessing) &&
                    styles.quickMatchButtonDisabledText,
                ]}>
          ...
          <View style={styles.cameraControls}>
            <TouchableOpacity
              style={[
                ...
                isCameraOn
                  ? styles.cameraControlActive
                  : styles.cameraControlInactive,
              ]}
            ...
            {isCameraOn && localStreamRef.current && (
              <TouchableOpacity
                style={[styles.cameraControlButton, styles.cameraControlActive]}
                onPress={flipCamera}>
                ...
                remoteStreamRef.current && isRemoteCameraOn ? (
                  <>
                    ...
                    <View style={styles.smallVideoIndicator}>
                      <Text style={styles.smallVideoIndicatorText}>
                        Stranger
                      </Text>
                    </View>
                  </>
